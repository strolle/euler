package se.strolle.euler.problems.p011;
...
public class GridProduct {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data[i].length; j++) {
				maxProduct = Math.max(maxProduct, calculateLeft(i, j));
				maxProduct = Math.max(maxProduct, calculateDown(i, j));
				maxProduct = Math.max(maxProduct, calculateDiagUp(i, j));
				maxProduct = Math.max(maxProduct, calculateDiagDown(i, j));
			}
		}
		System.out.println("Product : " + maxProduct);
	}

	private static long calculateLeft(int row, int col) {
		if (col + STEPS > data[row].length) {
			return -1L;
		}
		long prod = 1L;
	...
	private static long calculateDown(int row, int col) {
		if (row + STEPS > data.length) {
			return -1L;
		}
		long prod = 1L;
		for (int i = 0; i < STEPS; i++) {
	...
	private static long calculateDiagUp(int row, int col) {
		if (col + STEPS > data[row].length || row - STEPS < 0) {
			return -1L;
		}
		long prod = 1L;
		for (int i = 0; i < STEPS; i++) {
	...
	private static long calculateDiagDown(int row, int col) {
		if (col + STEPS > data[row].length || row + STEPS > data.length) {
			return -1L;
		}
		long prod = 1L;
		for (int i = 0; i < STEPS; i++) {
